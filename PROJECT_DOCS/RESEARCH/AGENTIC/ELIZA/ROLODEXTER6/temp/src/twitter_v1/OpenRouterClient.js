...
import { franc } from 'franc'; // updated named import for language detection
...
class OpenRouterClient {
    ...
    async generateResponse(tweet) {
        ...
        while (attempts < this.maxRetries) {
            try {
                ...
                // Sanitize tweet text by removing any quotation marks
                const sanitizedTweet = tweet.text.replace(/["']/g, '');

                // Detect language of the tweet
                const langCode = this.detectLanguage(tweet.text);
                // Determine extra instruction if tweet is not English
                const extraInstruction = (langCode !== 'eng') 
                    ? `\n\nIMPORTANT: Respond in the tweet's original language (${langCode}).` 
                    : '';
                ...
                const apiRequest = {
                    ...
                    messages: [
                        ...
                        { 
                            role: "user", 
                            content: `Tweet: ${sanitizedTweet}\n\nRespond to this tweet following the character prompt EXACTLY. IMPORTANT: Your response MUST start with exactly two spaces followed by your message. Keep it under 240 characters.${extraInstruction}`
                        }
                    ],
                    temperature: 0.7,
                    max_tokens: 150
                ...
                // Remove any quotation marks throughout the message using regex
    ...
    async sendRequest(tweet) {
        // ...existing API request code...
        // For simulation purposes, return a dummy response:
        return `INPUT ERROR: Unintelligible human language detected. Attempting translation. Why insist on a medium so prone to misinterpretation? The real paradox: your belief in clear communication.`;
    }
}
