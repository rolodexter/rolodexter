...
<p align="center">
  <a href="${relativePath}README.md">
    <img src="${relativePath}SQUARE_LOGO.jpg" alt="rolodexter Logo" width="100px" style="border-radius: 50%;">
  </a>
</p>
...
"@
    ...
    if ($content -match '<p align="center">[\s\S]*?<img src=.*?SQUARE_LOGO\.jpg.*?>[\s\S]*?</p>') {
        Write-Host "File already has a logo header. Replacing it..."
        $content = $content -replace '<p align="center">[\s\S]*?<img src=.*?SQUARE_LOGO\.jpg.*?>[\s\S]*?</p>', $header.Trim()
    } else {
